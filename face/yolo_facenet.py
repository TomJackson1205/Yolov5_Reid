...
class Detector(nn.Module):
    def __init__(self,img,person_idx,imgsz,half,weight,use_l_r=True):
    ...
    def forward(self,frame,conf_thres,iou_thres,face_thres=1.0):
        boxes=[]
        names = []
        all_images=[]
        gallery_loc = []
        ...
        for i, det in enumerate(pred):  # detections per image
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    # box = [np.maximum(x1-16,0), np.maximum(y1 - 16, 0), np.maximum(x2,frame.shape[0]), np.maximum(y2,frame.shape[1])]
                    box=[x1, y1, x2, y2]
                    face=crop_resize(frame,box,image_size=112)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if ret == False:
            break
        t=time.time()
        boxes, names = detect(frame[:,:,::-1], conf_thres=0.25, iou_thres=0.45)
